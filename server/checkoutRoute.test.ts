 ...
 *
 * 1. PRICE IS PLAN-DERIVED. The handler resolves the request's `plan` to a
 *    pinned price via resolvePlanTier + priceIdForTier and NEVER charges a
 *    client-supplied priceId — the /api/stripe/* routes publicly enumerate
 *    active price IDs, so trusting the body would let any caller check out
 *    against a cheaper active price.
 *
 * 2. NO SESSION-LEVEL TRIAL OVERRIDE. The advertised free trial (e.g.
 *    Kickstart's) lives on the Stripe PRICE (recurring.trial_period_days,
 *    guarded live by the credential health check's catalog probe). A trial
 *    key anywhere in the session params would silently replace the
 *    price-level trial and charge trial buyers immediately WITHOUT tripping
 *    the catalog health check, which only inspects the price object.
 */
...
import { PLAN_PRICING, LIVE_PRICE_IDS } from "@shared/stripe-constants";
...
const KICKSTART_PRICE_ID = LIVE_PRICE_IDS.kickstart;

describe("POST /api/stripe/checkout — trial contract", () => {
  it("creates the session against the plan's pinned price, quantity 1", async () => {
    const { handler, sessionsCreate } = makeDeps();
    const res = mockRes();
    await handler(mockReq({ plan: "kickstart", email: "buyer@example.com" }), res);
  ...
  it("never sets a session-level trial override anywhere in the session params", async () => {
    const { handler, sessionsCreate } = makeDeps();
    await handler(
      mockReq({
        plan: "kickstart",
  ...
  it("uses subscription mode for recurring prices (price-level trial flows through)", async () => {
    ...
    await handler(mockReq({ plan: "kickstart" }), mockRes());
  ...
  it("uses payment mode for one-time prices", async () => {
    ...
    await handler(mockReq({ plan: "transformation" }), mockRes());
    expect(sessionsCreate.mock.calls[0][0].mode).toBe("payment");
  });
});

describe("POST /api/stripe/checkout — plan-derived pricing (never trusts client priceId)", () => {
  it("ignores a mismatched client-supplied priceId and charges the plan's pinned price", async () => {
    const { handler, sessionsCreate, pricesRetrieve } = makeDeps();
    const res = mockRes();
    await handler(
      mockReq({ plan: "kickstart", priceId: "price_cheaper_evil", email: "buyer@example.com" }),
      res,
    );
    expect(res.statusCode).toBe(200);
    // Stripe is only ever queried for, and told to charge, the pinned price.
    expect(pricesRetrieve).toHaveBeenCalledWith(KICKSTART_PRICE_ID);
    expect(pricesRetrieve).not.toHaveBeenCalledWith("price_cheaper_evil");
    expect(sessionsCreate.mock.calls[0][0].line_items).toEqual([
      { price: KICKSTART_PRICE_ID, quantity: 1 },
    ]);
  });

  it("charges the transformation pinned price when plan=transformation, regardless of body priceId", async () => {
    const { handler, sessionsCreate, pricesRetrieve } = makeDeps({ recurring: null });
    await handler(mockReq({ plan: "transformation", priceId: KICKSTART_PRICE_ID }), mockRes());
    expect(pricesRetrieve).toHaveBeenCalledWith(LIVE_PRICE_IDS.transformation);
    expect(sessionsCreate.mock.calls[0][0].line_items).toEqual([
      { price: LIVE_PRICE_IDS.transformation, quantity: 1 },
    ]);
  });
});

describe("POST /api/stripe/checkout — request handling", () => {
  it("400s without a valid plan and never touches Stripe", async () => {
    const { handler, sessionsCreate, pricesRetrieve } = makeDeps();
    const res = mockRes();
    await handler(mockReq({ email: "buyer@example.com", priceId: KICKSTART_PRICE_ID }), res);
    expect(res.statusCode).toBe(400);
    expect(pricesRetrieve).not.toHaveBeenCalled();
    expect(sessionsCreate).not.toHaveBeenCalled();
  });

  it("400s on an untrusted plan string instead of charging a fallback tier", async () => {
    const { handler, sessionsCreate, leadStore } = makeDeps();
    const res = mockRes();
    await handler(
      mockReq({ plan: "<script>evil", priceId: KICKSTART_PRICE_ID, email: "buyer@example.com" }),
      res,
    );
    expect(res.statusCode).toBe(400);
    expect(sessionsCreate).not.toHaveBeenCalled();
  ...
  it("passes customer_email and truncated client_reference_id through", async () => {
    ...
    await handler(
      mockReq({ plan: "kickstart", email: "buyer@example.com", clientReferenceId: longRef }),
      mockRes(),
    );
    const params = sessionsCreate.mock.calls[0][0];
  ...
  it("passes the resolved plan through to lead capture", async () => {
    const { handler, leadStore } = makeDeps({ recurring: null });
    await handler(
      mockReq({ plan: "transformation", email: "buyer@example.com" }),
      mockRes(),
    );
  ...
  it("does not re-create an existing lead", async () => {
    ...
    await handler(mockReq({ plan: "kickstart", email: "buyer@example.com" }), mockRes());
    expect(leadStore.createLead).not.toHaveBeenCalled();
  });
  ...
  it("500s (without leaking details) when Stripe fails", async () => {
    ...
    const res = mockRes();
    await handler(mockReq({ plan: "kickstart" }), res);
