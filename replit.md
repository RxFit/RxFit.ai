...
- `scripts/validate-seo.mjs` — SEO/structured-data check for MDX posts (JSON-LD shapes, image files, default OG image) plus internal link validation (every `/blog/...` and root-relative link in MDX bodies must match an MDX slug or `shared/site.ts` STATIC_ROUTES); registered as the `seo` validation step. Also runs automatically inside `npm run build` (`script/build.ts` runs it right after vitest and aborts the build on non-zero exit), so broken links or invalid SEO data block every deploy.
