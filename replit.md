...
- `client/src/pages/AdminPage.tsx` — Internal `/admin` ops dashboard: prompts for the admin key (kept in sessionStorage only, sent as `x-admin-key` on every request — auth stays server-side), shows the per-service credential health snapshot with a "run check now" button, and the recent leads table. Noindexed; route is deliberately NOT in STATIC_ROUTES (so it never enters the sitemap or link validation) but is prerendered explicitly in `script/prerender.ts` and whitelisted in `server/vite.ts` `isKnownRoute` so it serves with HTTP 200; robots.txt adds `Disallow: /admin` to every UA group
